...
\begin{frame}{Two categorical variables with interaction}
    ...
    \[
    Y = \beta_0 + \beta_1 I_{\text{Med}} + \beta_2 I_{\text{Large}} + \beta_3 I_{\text{pub}} + \beta_4 I_{\text{Med}} I_{\text{pub}} + \beta_5 I_{\text{Large}} I_{\text{pub}} + \varepsilon
    \]
...
\begin{frame}{Categorical $\times$ continuous interaction}
    ...
    \[
    Y = \beta_0 + \beta_1 I_{\text{Med}} + \beta_2 I_{\text{Large}} + \beta_3 x + \beta_4 I_{\text{Med}} \cdot x + \beta_5 I_{\text{Large}} \cdot x + \varepsilon
    \]
...
\begin{frame}{Parallel vs.\ crossing lines}
    ...
    \begin{column}{0.48\textwidth}
    \centering
    \textbf{Without interaction}\\[0.3em]
    Same slope, different intercepts\\
    (parallel lines)\\[0.5em]
    $Y = \beta_0 + \beta_1 I_{\text{group}} + \beta_3 x$
    \end{column}
    \begin{column}{0.48\textwidth}
    \centering
    \textbf{With interaction}\\[0.3em]
    Different slopes per group\\
    (lines may cross)\\[0.5em]
    $Y = \beta_0 + \beta_1 I_{\text{group}} + \beta_3 x + \beta_4 I_{\text{group}} \cdot x$
    \end{column}
    \end{columns}
...
\begin{frame}{Categorical $\times$ continuous -- practice}
\begin{center}
Now, proceed to the example
\end{center}
\end{frame}
...
\subsection{Centering continuous variables}
...
\begin{frame}{Grand-mean centering vs.\ standardisation}
    \textbf{Grand-mean centering:} $x_c = x - \bar{x}$
    \begin{itemize}
        \item Shifts the zero-point to the sample mean
        \item Preserves original measurement units
    \end{itemize}

    \bigskip
    \textbf{Standardisation (z-score):} $x_s = (x - \bar{x}) / s_x$
    \begin{itemize}
        \item Shifts zero-point \emph{and} rescales to standard deviation units
        \item Coefficients become comparable across variables with different scales
        \item One-unit change = one-SD change
    \end{itemize}

    \bigskip
    Both preserve model fit ($R^2$, F-test). Only the \textbf{interpretation} of coefficients changes.
\end{frame}

\begin{frame}{Centering -- practice}
\begin{center}
Now, proceed to the example
\end{center}
\end{frame}
