...
Fitting a discrete distribution involves three basic steps:
    ...
    \justifying
        ...
        \item From this, we can calculate the fitted probabilities ($\hat{p}_k$) or equivalently, the model expected frequencies ($\hat{p}_kN$) [based on the CDF];
...
Often goodness-of-fit is examined with a classical (Pearson) goodness-of-fit (GOF) chisquare test
    ...
    where $K$ is number of levels and $s$ is number of parameters. An alternative test statistic is the likelihood-ratio $G^2$ statistic
    
    \begin{equation}
        G^{2}=\sum_{k=1}^{K} n_{k} \log \left(n_{k} / n \hat{p}_{k}\right) \sim \chi^2(K-s-1).
    \end{equation}
\end{frame}
...
\begin{frame}{Information Criteria}
    \begin{itemize}
        ...
        $$
         ...
         $$
         \text{LR} = -2\log\left(\frac{\hat{L}_1}{\hat{L}_2}\right) = 2\log(\hat{L}_2) - 2\log(\hat{L}_1) \sim \chi^2(k_2-k_1), 
         $$
         where $L_1$ is the log-lik of the first (reference) distribution and $L_2$ of the second, $k_1$ and $k_2$ denotes number of parameters of the first and second distribution.
    \end{itemize}
...
\begin{frame}{Procedures -- fitting specific distributions}
    \begin{table}[]
        ...
        \hline
         Packages  & 
         base, vcd, countreg & scipy.stats, pingouin &  Distributions.jl, HypothesisTests.jl\\
        Functions &
          chisq.test(), vcd::goodfit() &  chisquare() & ChisqTest()\\
        \hline
        \end{tabular}
    \end{table}
\end{frame}
...
\begin{frame}[fragile]{Example}
Generate two vectors $x \sim NB(3,2)$ and $y \sim Po(3)$
...
\alert{WARNING!} p-value calculated according to this method is incorrect! Why? See the degrees of freedom. It should be be 15 instead of 17 (NB has two parameters).
