...
Modify Model 3 (the releveled model) to use `woj = "14"` (Mazowieckie) as the reference
level instead of `"30"`. Compare the new intercept with the sample mean of
`vacancies` for `woj = "14"`. Are they equal? Why?
:::
...
Extract and compare the $R^2$ values from `model_treat`, `model_sum_size`, and
`model_poly`. Are they identical? Justify your answer based on the column space
of the design matrix under the three coding schemes.
:::
...
Fit the interaction model `vacancies ~ public * size` (variables in reverse
order). Compare the coefficient values with `model_int1`. Are the numbers
identical? Are the coefficient labels the same? Why might the labels differ?
:::
