...
Solution using `stats::optimize` -- maximization by default
...
We use the same zero-truncated Poisson data to compare five estimation methods: MLE, GMM, EM, Profile Likelihood, and QMLE. For the zero-truncated Poisson, the theoretical mean and variance are:

$$
E(X) = \frac{\lambda}{1 - e^{-\lambda}}, \quad V(X) = \frac{\lambda(1 + \lambda)}{1 - e^{-\lambda}} - \frac{\lambda^2}{(1 - e^{-\lambda})^2}.
$$
...
The method of moments matches the sample mean to the theoretical mean of the zero-truncated Poisson: $E(X) = \lambda / (1 - e^{-\lambda})$.
...
We treat zero-truncated Poisson as a complete-data Poisson with unobserved zeros. The EM algorithm iterates:

- **E-step**: estimate the expected number of missing zeros: $\hat{n}_0 = n \cdot e^{-\lambda} / (1 - e^{-\lambda})$
- **M-step**: update $\lambda = \sum x_i / (n + \hat{n}_0)$
...
For a single-parameter model, the profile likelihood is simply the log-likelihood evaluated over a grid of $\lambda$ values. We can use it to construct a confidence interval via the likelihood ratio: $\{  \lambda : 2[l(\hat{\lambda}) - l(\lambda)] \leq \chi^2_1(0.95) \}$.
...
Quasi-maximum likelihood only requires specifying the mean-variance relationship rather than the full distribution. For the zero-truncated Poisson, we solve the quasi-score equation using the mean function $\mu(\lambda) = \lambda / (1 - e^{-\lambda})$ and variance function $V(\mu) = \mu$ (Poisson-like).
