...
`fitdistrplus` package have PMF and CDF plots for assessing the fit.
...
1.  `goodfit` function returns a list with `fitted` (expected)
    frequencies under a given distribution using `pmf` of this
    distribution
...
Note that p-value is calculated incorrectly because `PowerDivergenceTest` does not account for estimated parameters. The correct df is 16, not 18 (18 categories minus $s=2$ estimated NB parameters). Corrected p-value:
