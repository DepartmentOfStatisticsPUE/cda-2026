...
1. Manually create $k-1 = 2$ dummy variables with `primary` as the reference level.
2. Declare `education` as a factor/categorical variable with `primary` as the reference level and inspect the model matrix.
...
Using the built-in `mtcars` dataset, focus on the `cyl` variable (number of cylinders: 4, 6, 8).

Where to find `mtcars`:

+ **R** -- built-in, load with `data(mtcars)` or use directly
+ **Python** -- available via `statsmodels`: `sm.datasets.get_rdataset("mtcars").data`
+ **Julia** -- available via `RDatasets.jl`: `dataset("datasets", "mtcars")`


1. Declare `cyl` as a factor/categorical variable and inspect the default model matrix. Which level is the reference? How many dummy columns are there and why?
2. Change the reference level to `8` and inspect the model matrix again. How do the coefficients change?
3. Apply effects (sum) coding and inspect the model matrix. How do the columns differ from (1)?
